...
/**
 * @file x64/sc_muladd.cpp
 * @brief x64 implementation of scalar multiply-add modulo l using Barrett reduction.
 *
 * Computes s = (a * b + c) mod l.
 * Uses 4x64-bit schoolbook multiplication, addend fusion, and Barrett reduction.
...
void sc_muladd_x64(unsigned char *s, const unsigned char *a, const unsigned char *b, const unsigned char *c)
{
    ...
    /* Step 1b: Add cv to product with carry propagation through all 8 limbs.
     * product = a*b + c. With 253-bit inputs: a*b < 2^506, c < 2^253,
     * so product < 2^506 + 2^253 which is within Barrett range. */
    {
        unsigned __int128 acc = (unsigned __int128)product[0] + cv[0];
        product[0] = (uint64_t)acc;
        acc = (unsigned __int128)product[1] + cv[1] + (uint64_t)(acc >> 64);
        product[1] = (uint64_t)acc;
        acc = (unsigned __int128)product[2] + cv[2] + (uint64_t)(acc >> 64);
        product[2] = (uint64_t)acc;
        acc = (unsigned __int128)product[3] + cv[3] + (uint64_t)(acc >> 64);
        product[3] = (uint64_t)acc;
        uint64_t carry = (uint64_t)(acc >> 64);
        acc = (unsigned __int128)product[4] + carry;
        product[4] = (uint64_t)acc;
        acc = (unsigned __int128)product[5] + (uint64_t)(acc >> 64);
        product[5] = (uint64_t)acc;
        acc = (unsigned __int128)product[6] + (uint64_t)(acc >> 64);
        product[6] = (uint64_t)acc;
        product[7] += (uint64_t)(acc >> 64);
    }
    ...
    /* Step 7: r = r1 - r2 (mod 2^256) */
    sub4(r, r1, r2);

    /* Step 8: Constant-time conditional subtract L, twice.
     * Same pattern as sc_add: select from d, pick r when borrow. */
    /* First subtraction */
    uint64_t borrow = sub4(d, r, L);
...
void sc_muladd_x64(unsigned char *s, const unsigned char *a, const unsigned char *b, const unsigned char *c)
{
     ...
     * so product < 2^506 + 2^253 which is within Barrett range. */
    {
        unsigned char carry = _addcarry_u64(0, product[0], cv[0], &product[0]);
        carry = _addcarry_u64(carry, product[1], cv[1], &product[1]);
        carry = _addcarry_u64(carry, product[2], cv[2], &product[2]);
        carry = _addcarry_u64(carry, product[3], cv[3], &product[3]);
        carry = _addcarry_u64(carry, product[4], 0, &product[4]);
        carry = _addcarry_u64(carry, product[5], 0, &product[5]);
        carry = _addcarry_u64(carry, product[6], 0, &product[6]);
        _addcarry_u64(carry, product[7], 0, &product[7]);
    }
    ...
    /* Step 7: r = r1 - r2 (mod 2^256) using _subborrow_u64 */
    unsigned char borrow = 0;
    borrow = _subborrow_u64(borrow, r1[0], r2[0], &r[0]);
    borrow = _subborrow_u64(borrow, r1[1], r2[1], &r[1]);
    borrow = _subborrow_u64(borrow, r1[2], r2[2], &r[2]);
    _subborrow_u64(borrow, r1[3], r2[3], &r[3]);
