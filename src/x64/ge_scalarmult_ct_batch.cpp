...
/**
 * @file x64/ge_scalarmult_ct_batch.cpp
 * @brief x64 fallback for batch CT scalar multiplication (loop over single ops).
 */
...
void ge_scalarmult_ct_batch_x64(ge_p2 *results, const unsigned char *scalars, const ge_p3 *points, size_t count)
{
    ...
    {
        ge_p1p1 t;
        ge_scalarmult_x64_ct(&t, scalars + i * 32, &points[i]);
