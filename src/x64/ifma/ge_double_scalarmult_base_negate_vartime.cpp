...
/**
 * @file ifma/ge_double_scalarmult_base_negate_vartime.cpp
 * @brief AVX-512 IFMA variable-time double scalar multiplication with base point.
 *
...
static FE_IFMA_FORCE_INLINE void ge_msub_ifma(ge_p1p1 *r, const ge_p3 *p, const ge_precomp *q)
{
    fe t0;
    fe_add(r->X, p->Y, p->X);
    fe_sub(r->Y, p->Y, p->X);
...
// Base point multiples in ge_precomp form (same table as x64 version)
    ...
    const unsigned char *b)
{
    ...
    for (; i >= 0; --i)
    {
        ...
        {
            ge_p1p1_to_p3_ifma(&u, t);
            ge_madd_ifma(t, &u, &ge_Bi[bslide[i] / 2]);
        }
        else if (bslide[i] < 0)
        {
            ge_p1p1_to_p3_ifma(&u, t);
            ge_msub_ifma(t, &u, &ge_Bi[(-bslide[i]) / 2]);
        }
