...
/**
 * @file ifma/ge_dsm_negate_vt_batch_ss.cpp
 * @brief IFMA 8-way batch double scalar multiplication with shared scalars.
 *
 * Computes results[i] = a * A_points[i] + b * (-B_points[i]) where a single
 * scalar pair (a, b) is shared across all operations.
 *
 * The shared-scalar property eliminates the 8-iteration cmov selection loops
 * that dominate the per-lane-scalar batch: since all lanes process the same
 * scalar digit, table lookups use direct indexing (a single load + conditional
...
static const fe ge_dsm_batch_ss_ifma_d2_fe51 =
    ...
    size_t count)
{
    ...
    fe51x8 d2_8x;
    fe51x8_broadcast_d2(&d2_8x, ge_dsm_batch_ss_ifma_d2_fe51);
    ...
    for (size_t batch = 0; batch < count; batch += 8)
    {
        ...
        for (int d = 63; d >= 0; d--)
        {
            ...
            // Use add/sub to avoid struct copy + cneg (works around MSVC ICE)
            {
                int a_digit = ae[d];
                if (a_digit != 0)
                {
