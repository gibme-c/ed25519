...
/**
 * @file avx2/ge_dsm_negate_vt_batch_ss.cpp
 * @brief AVX2 4-way batch double scalar multiplication with shared scalars.
 *
...
static const fe ge_dsm_batch_ss_d2_fe51 =
    ...
    size_t count)
{
    ...
    fe10x4 d2_4x;
    fe10x4_broadcast_d2(&d2_4x, ge_dsm_batch_ss_d2_fe51);
    ...
    for (size_t batch = 0; batch < count; batch += 4)
    {
        ...
        for (int d = 63; d >= 0; d--)
        {
            ...
            // Use add/sub to avoid struct copy + cneg
            {
                int a_digit = ae[d];
                if (a_digit != 0)
                {
