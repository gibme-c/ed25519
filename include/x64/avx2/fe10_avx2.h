 ...
 * the 51-bit × 51-bit products. On MSVC, the _umul128 results are returned
 * through an ed25519_uint128_emu struct, which the optimizer can't keep in
 * registers -- so force-inlining fe_mul into ge bodies causes massive register
 * spilling and a 15-30% regression. The radix-2^25.5 representation sidesteps
 * this entirely: products are at most 26 × 26 = 52 bits, fitting in plain
 ...
 *
 * Also provides fe51-to-fe10 and fe10-to-fe51 conversion functions, and
 * the fe10-based ge point types (ge_p2_10, ge_p3_10, etc.) used by the
 * AVX2 fe10-throughout scalarmult path.
 */

#ifndef ED25519_X64_AVX2_FE10_AVX2_H
#define ED25519_X64_AVX2_FE10_AVX2_H
...
#if defined(_MSC_VER)
#define FE10_AVX2_FORCE_INLINE __forceinline
#else
#define FE10_AVX2_FORCE_INLINE inline __attribute__((always_inline))
#endif

typedef int64_t fe10[10];

static const int64_t FE10_MASK26 = (1LL << 26) - 1;
static const int64_t FE10_MASK25 = (1LL << 25) - 1;

/**
 * @brief Convert fe51 (radix-2^51, uint64_t[5]) to fe10 (radix-2^25.5, int64_t[10]).
 *
 * Carry-propagates the fe51 input first to ensure each limb is ≤51 bits,
 ...
 */
static FE10_AVX2_FORCE_INLINE void fe51_to_fe10(fe10 out, const fe src)
{
...
/**
 * @brief Convert fe10 (radix-2^25.5, int64_t[10]) to fe51 (radix-2^51, uint64_t[5]).
 *
 * Performs carry propagation first to ensure limbs are in canonical range,
 * then packs pairs of limbs back into 51-bit limbs.
 */
static FE10_AVX2_FORCE_INLINE void fe10_to_fe51(fe out, const fe10 src)
{
    int64_t t[10], c;
    ...
    //   t[0]: 26 bits, t[1]: 25 bits → 26+25 = 51 bits → fe51[0]
    //   t[2]: 26 bits, t[3]: 25 bits → fe51[1]
    //   t[4]: 26 bits, t[5]: 25 bits → fe51[2]
    //   t[6]: 26 bits, t[7]: 25 bits → fe51[3]
    //   t[8]: 26 bits, t[9]: 25 bits → fe51[4]
...
/**
 * @brief fe10 addition: h = f + g (no carry propagation).
 */
static FE10_AVX2_FORCE_INLINE void fe10_add(fe10 h, const fe10 f, const fe10 g)
{
    h[0] = f[0] + g[0];
    h[1] = f[1] + g[1];
    h[2] = f[2] + g[2];
...
/**
 * @brief fe10 subtraction: h = f - g with bias to keep positive.
 */
static FE10_AVX2_FORCE_INLINE void fe10_sub(fe10 h, const fe10 f, const fe10 g)
{
...
/**
 * @brief fe10 negation: h = -f (mod p).
 */
static FE10_AVX2_FORCE_INLINE void fe10_neg(fe10 h, const fe10 f)
{
    fe10 zero = {0};
    fe10_sub(h, zero, f);
}
 ...
 * @brief fe10 copy: h = f.
 */
static FE10_AVX2_FORCE_INLINE void fe10_copy(fe10 h, const fe10 f)
{
    h[0] = f[0];
    h[1] = f[1];
    h[2] = f[2];
...
/**
 * @brief fe10 conditional move: if b, then t = u.
 */
static FE10_AVX2_FORCE_INLINE void fe10_cmov(fe10 t, const fe10 u, int64_t b)
{
    int64_t mask = -b;
    t[0] ^= mask & (t[0] ^ u[0]);
    t[1] ^= mask & (t[1] ^ u[1]);
...
/**
 * @brief fe10 schoolbook multiplication: h = f * g (mod 2^255-19).
 *
 ...
 *
 * This is essentially the same algorithm as portable/fe_mul but using int64_t
 * accumulators instead of int32_t limbs and int64_t products. The key
 * advantage over fe51_mul for MSVC: no 128-bit arithmetic, so no
 * ed25519_uint128_emu struct, making it safe to force-inline.
 */
static FE10_AVX2_FORCE_INLINE void fe10_mul_avx2(fe10 h, const fe10 f, const fe10 g)
{
...
/**
 * @brief fe10 squaring: h = f^2 (mod 2^255-19).
 */
static FE10_AVX2_FORCE_INLINE void fe10_sq_avx2(fe10 h, const fe10 f)
{
    int64_t f0 = f[0], f1 = f[1], f2 = f[2], f3 = f[3], f4 = f[4];
    int64_t f5 = f[5], f6 = f[6], f7 = f[7], f8 = f[8], f9 = f[9];
...
/**
 * @brief fe10 double-squaring: h = 2 * f^2 (mod 2^255-19).
 */
static FE10_AVX2_FORCE_INLINE void fe10_sq2_avx2(fe10 h, const fe10 f)
{
    int64_t f0 = f[0], f1 = f[1], f2 = f[2], f3 = f[3], f4 = f[4];
    int64_t f5 = f[5], f6 = f[6], f7 = f[7], f8 = f[8], f9 = f[9];
...
/**
 * @brief fe10 repeated squaring: h = f^(2^n) (mod 2^255-19).
 */
static FE10_AVX2_FORCE_INLINE void fe10_sqn_avx2(fe10 h, const fe10 f, int n)
{
    fe10_sq_avx2(h, f);
    for (int i = 1; i < n; i++)
        fe10_sq_avx2(h, h);
...
/**
 * @brief fe10 group element types for the AVX2 fe10-throughout path.
 *
 * Each coordinate is an fe10 (10 x int64_t = 80 bytes) instead of the usual
 * fe (5 x uint64_t = 40 bytes). This doubles the memory footprint per point,
 * but eliminates the ~25 ops per fe51-to-fe10 conversion that would otherwise
 * happen inside every ge operation. The MSVC AVX2 scalarmult TUs convert
 * fe51 -> fe10 once at entry, run the entire scalar multiplication loop in
 * fe10, and convert back at exit.
 */
typedef struct
{
    fe10 X, Y, Z;
...
typedef struct
{
    fe10 YplusX, YminusX, Z, T2d;
} ge_cached_10;

/** @brief fe10 precomp (affine Z=1) for a single selected base-point table entry. */
typedef struct
{
    fe10 yplusx, yminusx, xy2d;
} ge_precomp_10;

#endif // ED25519_X64_AVX2_FE10_AVX2_H
