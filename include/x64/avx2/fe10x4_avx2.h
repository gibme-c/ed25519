 ...
 * Each fe10x4 holds 4 independent field elements packed horizontally into
 * AVX2 registers -- one element per 64-bit lane, 10 registers per fe10x4
 * (one per radix-2^25.5 limb). The representation is the same alternating
 * 26/25-bit unsigned limb layout used by the scalar fe10 in fe10_avx2.h.
 *
 ...
 * schoolbook product follows the same formula as fe10_mul_avx2 in
 * fe10_avx2.h, with pre-multiplied 19*g terms for the wrap-around and
 * pre-doubled odd-indexed f limbs to compensate for the alternating radix.
 *
 * Compared to the IFMA 8-way variant (fe51x8_ifma.h), the AVX2 path uses
 * twice as many registers per field element (10 vs 5 limbs) and processes
 * half as many operations per batch (4 vs 8 lanes), but works on any CPU
 * with AVX2 support -- no AVX-512 required.
 *
...
#ifndef ED25519_X64_AVX2_FE10X4_AVX2_H
#define ED25519_X64_AVX2_FE10X4_AVX2_H
...
#if defined(_MSC_VER)
#define FE10X4_FORCE_INLINE __forceinline
#else
#define FE10X4_FORCE_INLINE inline __attribute__((always_inline))
#endif
...
static inline __m256i fe10x4_mask26(void)
{
    return _mm256_set1_epi64x((1LL << 26) - 1);
}
static inline __m256i fe10x4_mask25(void)
{
    return _mm256_set1_epi64x((1LL << 25) - 1);
}
static inline __m256i fe10x4_c19(void)
{
    return _mm256_set1_epi64x(19);
}
static inline __m256i fe10x4_bias0(void)
{
    return _mm256_set1_epi64x(0x7FFFFDA);
}
static inline __m256i fe10x4_bias_even(void)
{
    return _mm256_set1_epi64x(0x7FFFFFE);
}
static inline __m256i fe10x4_bias_odd(void)
{
    return _mm256_set1_epi64x(0x3FFFFFE);
}

#define FE10X4_MASK26 fe10x4_mask26()
#define FE10X4_MASK25 fe10x4_mask25()
#define FE10X4_19 fe10x4_c19()
#define FE10X4_BIAS0 fe10x4_bias0()
#define FE10X4_BIAS_EVEN fe10x4_bias_even()
#define FE10X4_BIAS_ODD fe10x4_bias_odd()

/**
 * @brief Zero all 4 field elements.
 */
static FE10X4_FORCE_INLINE void fe10x4_0(fe10x4 *h)
{
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_1(fe10x4 *h)
{
    const __m256i z = _mm256_setzero_si256();
 ...
 * @brief Copy: h = f.
 */
static FE10X4_FORCE_INLINE void fe10x4_copy(fe10x4 *h, const fe10x4 *f)
{
    for (int i = 0; i < 10; i++)
        h->v[i] = f->v[i];
}
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_add(fe10x4 *h, const fe10x4 *f, const fe10x4 *g)
{
    for (int i = 0; i < 10; i++)
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_sub(fe10x4 *h, const fe10x4 *f, const fe10x4 *g)
{
    // Add 2*p bias and subtract g
    h->v[0] = _mm256_sub_epi64(_mm256_add_epi64(f->v[0], FE10X4_BIAS0), g->v[0]);
    h->v[1] = _mm256_sub_epi64(_mm256_add_epi64(f->v[1], FE10X4_BIAS_ODD), g->v[1]);
    h->v[2] = _mm256_sub_epi64(_mm256_add_epi64(f->v[2], FE10X4_BIAS_EVEN), g->v[2]);
    h->v[3] = _mm256_sub_epi64(_mm256_add_epi64(f->v[3], FE10X4_BIAS_ODD), g->v[3]);
    h->v[4] = _mm256_sub_epi64(_mm256_add_epi64(f->v[4], FE10X4_BIAS_EVEN), g->v[4]);
    h->v[5] = _mm256_sub_epi64(_mm256_add_epi64(f->v[5], FE10X4_BIAS_ODD), g->v[5]);
    h->v[6] = _mm256_sub_epi64(_mm256_add_epi64(f->v[6], FE10X4_BIAS_EVEN), g->v[6]);
    h->v[7] = _mm256_sub_epi64(_mm256_add_epi64(f->v[7], FE10X4_BIAS_ODD), g->v[7]);
    h->v[8] = _mm256_sub_epi64(_mm256_add_epi64(f->v[8], FE10X4_BIAS_EVEN), g->v[8]);
    h->v[9] = _mm256_sub_epi64(_mm256_add_epi64(f->v[9], FE10X4_BIAS_ODD), g->v[9]);
    ...
    h->v[0] = _mm256_and_si256(h->v[0], FE10X4_MASK26);

    c = _mm256_srli_epi64(h->v[1], 25);
    h->v[2] = _mm256_add_epi64(h->v[2], c);
    h->v[1] = _mm256_and_si256(h->v[1], FE10X4_MASK25);

    c = _mm256_srli_epi64(h->v[2], 26);
    h->v[3] = _mm256_add_epi64(h->v[3], c);
    h->v[2] = _mm256_and_si256(h->v[2], FE10X4_MASK26);

    c = _mm256_srli_epi64(h->v[3], 25);
    h->v[4] = _mm256_add_epi64(h->v[4], c);
    h->v[3] = _mm256_and_si256(h->v[3], FE10X4_MASK25);

    c = _mm256_srli_epi64(h->v[4], 26);
    h->v[5] = _mm256_add_epi64(h->v[5], c);
    h->v[4] = _mm256_and_si256(h->v[4], FE10X4_MASK26);

    c = _mm256_srli_epi64(h->v[5], 25);
    h->v[6] = _mm256_add_epi64(h->v[6], c);
    h->v[5] = _mm256_and_si256(h->v[5], FE10X4_MASK25);

    c = _mm256_srli_epi64(h->v[6], 26);
    h->v[7] = _mm256_add_epi64(h->v[7], c);
    h->v[6] = _mm256_and_si256(h->v[6], FE10X4_MASK26);

    c = _mm256_srli_epi64(h->v[7], 25);
    h->v[8] = _mm256_add_epi64(h->v[8], c);
    h->v[7] = _mm256_and_si256(h->v[7], FE10X4_MASK25);

    c = _mm256_srli_epi64(h->v[8], 26);
    h->v[9] = _mm256_add_epi64(h->v[9], c);
    h->v[8] = _mm256_and_si256(h->v[8], FE10X4_MASK26);

    c = _mm256_srli_epi64(h->v[9], 25);
    h->v[0] = _mm256_add_epi64(h->v[0], _mm256_mul_epu32(c, FE10X4_19));
    h->v[9] = _mm256_and_si256(h->v[9], FE10X4_MASK25);
}

/**
 * @brief Negation: h = -f (mod p).
 */
static FE10X4_FORCE_INLINE void fe10x4_neg(fe10x4 *h, const fe10x4 *f)
{
    fe10x4 zero;
    fe10x4_0(&zero);
    fe10x4_sub(h, &zero, f);
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_carry(fe10x4 *h)
{
    __m256i c;
 ...
 *
 * Vectorized version of fe10_mul_avx2. Uses _mm256_mul_epu32 for 32x32->64
 * unsigned products. Limbs are at most 26 bits, so all products fit in 64 bits.
 */
static FE10X4_FORCE_INLINE void fe10x4_mul(fe10x4 *h, const fe10x4 *f, const fe10x4 *g)
{
    ...
    const __m256i g1_19 = _mm256_mul_epu32(g1, FE10X4_19);
    const __m256i g2_19 = _mm256_mul_epu32(g2, FE10X4_19);
    const __m256i g3_19 = _mm256_mul_epu32(g3, FE10X4_19);
    const __m256i g4_19 = _mm256_mul_epu32(g4, FE10X4_19);
    const __m256i g5_19 = _mm256_mul_epu32(g5, FE10X4_19);
    const __m256i g6_19 = _mm256_mul_epu32(g6, FE10X4_19);
    const __m256i g7_19 = _mm256_mul_epu32(g7, FE10X4_19);
    const __m256i g8_19 = _mm256_mul_epu32(g8, FE10X4_19);
    const __m256i g9_19 = _mm256_mul_epu32(g9, FE10X4_19);
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_sq(fe10x4 *h, const fe10x4 *f)
{
    const __m256i f0 = f->v[0], f1 = f->v[1], f2 = f->v[2], f3 = f->v[3], f4 = f->v[4];
    const __m256i f5 = f->v[5], f6 = f->v[6], f7 = f->v[7], f8 = f->v[8], f9 = f->v[9];
    ...
    const __m256i f6_19 = _mm256_mul_epu32(f6, FE10X4_19);
    const __m256i f7_38 = _mm256_mul_epu32(f7, v38);
    const __m256i f8_19 = _mm256_mul_epu32(f8, FE10X4_19);
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_sq2(fe10x4 *h, const fe10x4 *f)
{
    fe10x4_sq(h, f);
    for (int i = 0; i < 10; i++)
 ...
 */
static FE10X4_FORCE_INLINE void fe10x4_cmov(fe10x4 *t, const fe10x4 *u, __m256i mask)
{
    for (int i = 0; i < 10; i++)
...
/**
 * @brief Pack a single fe10 into one lane of a fe10x4.
 *
 * Sets lane `lane` of each register in `out` to the corresponding limb of `in`.
 * Other lanes are unchanged. lane must be 0..3.
 */
static FE10X4_FORCE_INLINE void fe10x4_insert_lane(fe10x4 *out, const fe10 in, int lane)
{
...
/**
 * @brief Extract one lane from a fe10x4 into a scalar fe10.
 */
static FE10X4_FORCE_INLINE void fe10x4_extract_lane(fe10 out, const fe10x4 *in, int lane)
{
    alignas(32) int64_t tmp[4];
    for (int i = 0; i < 10; i++)
    {
...
/**
 * @brief Pack 4 fe10 values into a fe10x4.
 */
static FE10X4_FORCE_INLINE void fe10x4_pack(fe10x4 *out, const fe10 a, const fe10 b, const fe10 c, const fe10 d)
{
    for (int i = 0; i < 10; i++)
...
/**
 * @brief Unpack a fe10x4 into 4 fe10 values.
 */
static FE10X4_FORCE_INLINE void fe10x4_unpack(fe10 a, fe10 b, fe10 c, fe10 d, const fe10x4 *in)
{
    alignas(32) int64_t tmp[4];
    for (int i = 0; i < 10; i++)
    {
...
#endif // ED25519_X64_AVX2_FE10X4_AVX2_H
